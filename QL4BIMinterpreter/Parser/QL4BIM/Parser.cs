...
public FunctionNode GlobalBlock { get; set; }
	...
	void statement_() {
		...
		Expect(3);
		OnParsed(ParserParts.VariableBegin, t.val); 
		while (la.kind == 8 || la.kind == 9) {
			if (la.kind == 8) {
				Get();
				OnParsed(ParserParts.VariableEmptyRelAtt); 
			} else {
				Get();
				Expect(3);
				OnParsed(ParserParts.VariableRelAtt, t.val); 
				Expect(10);
				Expect(3);
				OnParsed(ParserParts.VariableRelAtt, t.val); 
				while (la.kind == 10) {
		...
		OnParsed(ParserParts.VariableEnd, t.val); 
		Expect(12);
		expression();
	}
	...
	void setRelAttLongShort() {
		if (la.kind == 3) {
			Get();
			OnParsed(ParserParts.ArguementSetRelBegin, t.val); 
			if (la.kind == 9) {
				relAtt();
			}
		} else if (la.kind == 9) {
	...
	void relAtt() {
		Expect(9);
		Expect(3);
		OnParsed(ParserParts.ArguementRelAtt, t.val); 
		Expect(11);
	}
	...
	void setRelFormalArg() {
		...
		OnParsed(ParserParts.ArgumentRelSetEnd, t.val); 
	}
